...
//		System.out.println(itemList);
		...
		ArrayList<Item> highPriceList = (ArrayList<Item>) itemList.stream()
				.filter(price -> price.getPrice() >= 10000)
		...
		ArrayList<String> meatNames = (ArrayList<String>) itemList.stream().distinct()
				.filter(category -> category.getCategory().equals("고기")).map(Item::getName).collect(Collectors.toList());
